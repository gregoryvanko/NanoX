...
module.exports = (req, res, next) => {
    ...
    .then(user => {
        if (user != null){
            ...
            return res.status(401).send({Error: true, ErrorMsg: "User not found by id in Admin Auth"})
        }
    })
    .catch((err) => {
        LogError(`Mongoose find error in Auth Admin: ${err.message}`)
        return res.status(401).send({Error: true, ErrorMsg: "Mongoose find error in Admin Auth"})
    })
}